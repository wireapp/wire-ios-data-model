...
enum InvalidConversationRemoval {
    ...
    static func removeInvalid(in moc: NSManagedObjectContext) {
        do {
            try moc.batchDeleteEntities(named: ZMConversation.entityName(), matching: NSPredicate(format: "\(ZMConversation.ZMConversationConversationTypeKey) == \(ZMConversationType.invalid.rawValue)"))
        } catch {
